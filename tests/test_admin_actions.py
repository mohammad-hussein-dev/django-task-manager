...
def test_admin_mark_as_completed():
    ...
    admin.mark_as_completed(request, Task.objects.filter(pk__in=[task1.pk, task2.pk]))
...
def test_admin_mark_as_pending():
    ...
    admin.mark_as_pending(request, Task.objects.filter(pk__in=[task1.pk, task2.pk]))
    task1.refresh_from_db()
    task2.refresh_from_db()
